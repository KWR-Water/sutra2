...
1. Define the hydrogeochemical system using the HydroChemicalSchematisation class.
In the HydroChemicalSchematisation class choose the:
...
* The removal function ('omp', 'microbiology)
* Input the relevant geohydrological data

2. Run the AnalyticalWell class to calculate the travel time distribution in the different aquifer zones

3. Run the Substance class to retrieve the substance (removal) paramters

4. Run the Concentration class to calculate the removal and concentration in each zone and in the well

5. Plot the desired functions 
...
.. code-block:: python
    ...
    import greta.draft_transport_function
    import pandas as pd
    from pathlib import Path

    #AH what of below do we need?
...
Example: phreatic .\\research\phreatic_implementation_example.py
        semiconfined .\\research\semi_confined_implementation_example.py
...
In this example we calculate analytical solution for a phreatic well, with a diffuse 
...
Step 4: Run the Concentration class 
=====================================
To calculate the removal and the steady-state concentration in each zone, create a concentration 
object by running the Concentration class with the phreatic_well object and specifying
